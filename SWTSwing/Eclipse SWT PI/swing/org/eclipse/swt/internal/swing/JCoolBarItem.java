 ...
 */
public class JCoolBarItem extends JPanel {
   ...
   */  
  public JCoolBarItem(boolean isWrapped) {
   ...
   */  
  public JCoolBarItem(JToolBar toolBar) {
    this(toolBar, false);
  }
 ...
 *----------------------------------------------------------------------------*/
  ...
  private JCoolBar getCoolBar() {
    return (JCoolBar)JCoolBarItem.this.getParent();
  }
  ...
  private ItemInformation getItemInformation() {
    ...
    for(int i=0; i<coolBar.getComponentCount(); i++) {
      JCoolBarItem item = (JCoolBarItem)coolBar.getComponent(i);
      if(!itemFound) {
        if(item.isWrapped()) {
          indent = 0;
        }
...
//          location = newLocation;
          ...
          if(isUp) {
            ...
            if(itemInformation.startRowItem == itemInformation.index && itemInformation.endRowItem == itemInformation.index) {
              ...
              if(comp instanceof JCoolBarItem) {
                JCoolBarItem item = (JCoolBarItem)comp;
                for(int i=itemInformation.startRowItem - 1; i>=0; i--) {
                  if(coolBar.getComponent(i) == item) {
                    ...
                    if(item.isInToolBar(itemPoint)) {
                      setIndent(0);
                      coolBar.add(JCoolBarItem.this, i + 1);
                      setWrapped(false);
                    } else {
                      ...
                      JCoolBarItem.this.setWrapped(item.isWrapped());
                      item.setWrapped(false);
                      coolBar.add(JCoolBarItem.this, i);
                    }
                    break;
                  }
                }
                ...
                setWrapped(false);
                coolBar.add(JCoolBarItem.this, itemInformation.startRowItem);
                setIndent(2*getIndent() - getItemInformation().totalIndent);
              }
            } else {
              setWrapped(true);
              coolBar.add(JCoolBarItem.this, itemInformation.startRowItem);
              ((JCoolBarItem)coolBar.getComponent(itemInformation.startRowItem + 1)).setWrapped(true);
            ...
            if(itemInformation.startRowItem == itemInformation.index && itemInformation.endRowItem == itemInformation.index) {
              ...
              if(comp instanceof JCoolBarItem) {
                ...
                for(int i=itemInformation.endRowItem + 1; i<coolBar.getComponentCount(); i++) {
                  if(coolBar.getComponent(i) == item) {
                    ...
                    if(item.isInToolBar(itemPoint)) {
                      // on bar
                      setIndent(0);
                      coolBar.add(JCoolBarItem.this, item.getItemInformation().index);
                      setWrapped(false);
                    } else {
                      // on rest of bar
                      ItemInformation itemInfo = item.getItemInformation();
                      ...
                      coolBar.add(JCoolBarItem.this, itemInfo.index - 1);
                      setIndent(2*getIndent() - getItemInformation().totalIndent);
                    }
                    break;
                  }
                ...
                if(itemInformation.endRowItem + 1 < coolBar.getComponentCount()) {
                  JCoolBarItem item = (JCoolBarItem)coolBar.getComponent(itemInformation.endRowItem + 1);
                  ItemInformation itemInfo = item.getItemInformation();
                  coolBar.add(JCoolBarItem.this, itemInfo.endRowItem);
                  setWrapped(false);
              ...
              setWrapped(true);
              coolBar.add(JCoolBarItem.this, itemInformation.endRowItem);
              setIndent(itemInformation.totalIndent);
              if(itemInformation.startRowItem == itemInformation.index) {
                ((JCoolBarItem)coolBar.getComponent(itemInformation.index)).setWrapped(true);
              }
            }
          }
          revalidate();
        ...
        if(diffX < 0) {
          ...
          if(itemInformation.index != itemInformation.endRowItem) {
            JCoolBarItem item = null;
            for(int i=itemInformation.index+1; i<=itemInformation.endRowItem && item == null; i++) {
              JCoolBarItem tempItem = (JCoolBarItem)coolBar.getComponent(i);
          ...
          if(item != JCoolBarItem.this) {
            ...
            if(item.isInBumps(coolItemPoint)) {
              ...
              if(index < itemInformation.endRowItem) {
                ((JCoolBarItem)coolBar.getComponent(index + 1)).setWrapped(isWrapped());
              }
              setWrapped(item.isWrapped());
              item.setWrapped(false);
              coolBar.add(JCoolBarItem.this, itemIndex<index? itemIndex: itemIndex - 1);
              // TODO: set wrapping
 ...
 *----------------------------------------------------------------------------*/
   ...
   * @param isWrapped Indicate if the item is wrapped.
   */  
  public JCoolBarItem(JToolBar toolBar, boolean isWrapped) {
...
////    toolBar.addMouseListener(dragListener);
    ...
    toolBarPanel.addContainerListener(new java.awt.event.ContainerAdapter() {
      public void componentAdded(java.awt.event.ContainerEvent e) {
        isExtracted = false;
        JCoolBarItem.this.setVisible(true);
      }
      public void componentRemoved(java.awt.event.ContainerEvent e) {
        isExtracted = true;
        JCoolBarItem.this.setVisible(false);
      }
    });
