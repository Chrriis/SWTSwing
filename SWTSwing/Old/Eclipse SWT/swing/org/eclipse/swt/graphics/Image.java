...
public final class Image implements Drawable2 {

  // TODO: remove this win32 implementation...
   ...
   * (Warning: This field is platform dependent)
   */
  public java.awt.Image swingHandle;
  ...
  public Image(Device device, int width, int height) {
    ...
    // TODO: remove this bypass and actually implement the constructor!
    if(true) {
      swingHandle = new java.awt.image.BufferedImage(width, height, java.awt.image.BufferedImage.TYPE_INT_RGB);
      this.device = device;
      if (device.tracking)
      ...
      case SWT.IMAGE_DISABLE :
        {
          Rectangle r = srcImage.getBounds();
          this.type = srcImage.type;
          byte[] rgbBwBitmapInfo = { 40, 0, 0, 0, /* biSize */
          ...
          switch (type) {
            ...
            case SWT.ICON :
              ...
              int newHbmp =
                OS.CreateCompatibleBitmap(hdcSource, r.width, r.height);
              if (newHbmp == 0)
                SWT.error(SWT.ERROR_NO_HANDLES);
      ...
      case SWT.IMAGE_GRAY :
        {
          ...
          if (!palette.isDirect) {
            ...
            newData =
              new ImageData(r.width, r.height, 8, new PaletteData(rgbs));
            ...
            for (int y = 0; y < r.height; y++) {
              ...
              for (int x = 0; x < r.width; x++) {
                ...
                blue =
                  (blueShift < 0) ? blue >>> -blueShift : blue << blueShift;
                newData.data[offset++] =
                  (byte) ((red
                    + red
                    + green
  ...
  public Image(Device device, ImageData data) {
    ...
    // TODO: remove this ugly stuff because it is really gready: saving data to read it again... bah!
    ImageLoader il = new ImageLoader();
    il.data = new ImageData[] {data};
    java.io.ByteArrayOutputStream baos = new java.io.ByteArrayOutputStream();
    il.save(baos, SWT.IMAGE_GIF);
    swingHandle = new javax.swing.ImageIcon(baos.toByteArray()).getImage();
    this.device = device;
    if (device.tracking)
      device.new_Object(this);
    if(true)
  ...
  public Image(Device device, ImageData source, ImageData mask) {
    ...
    ImageData newMask =
      new ImageData(mask.width, mask.height, 1, mask.palette, 2, newMaskData);
    int[] maskPixels = new int[mask.width];
    for (int y = 0; y < mask.height; y++) {
      mask.getPixels(0, y, mask.width, maskPixels, 0);
  ...
  public Image(Device device, InputStream stream) {
    ...
    try {
      this.device = device;
      swingHandle = ImageIO.read(stream);
//      swingHandle = ImageIO.read(new ByteArrayInputStream(source.data));
    } catch(Exception e) {
      e.printStackTrace();
    }
    this.device = device;
...
//    }
  ...
  public Color getBackground() {
    ...
    if (bm.bmBitsPixel <= 8) {
      if (OS.IsWinCE) {
        ...
        pBits[0] =
          (byte) ((transparentPixel << (8 - bm.bmBitsPixel))
            | (pBits[0] & ~mask));
    ...
    /* Release the HDC for the device */
    device.internal_dispose_GC(hDC, null);
    return Color.win32_new(
      device,
      0x02000000 | (blue << 16) | (green << 8) | red);
  }
  ...
  public Rectangle getBounds() {
    ...
    return new Rectangle(0, 0, swingHandle.getWidth(null), swingHandle.getHeight(null));
...
//    // This is just an attempt/investigation on ImageIO capabilities. It is not a solution...
//    if(true) {
//      javax.imageio.ImageWriter imageWriter = (javax.imageio.ImageWriter)javax.imageio.ImageIO.getImageWritersByFormatName("jpeg").next();
//      java.io.ByteArrayOutputStream baos = new java.io.ByteArrayOutputStream();
//      try {
//        javax.imageio.stream.ImageOutputStream ios = javax.imageio.ImageIO.createImageOutputStream(baos);
//        imageWriter.setOutput(ios);
//        imageWriter.write((java.awt.image.RenderedImage)swingHandle);
//      } catch(Exception e) {
//        e.printStackTrace();
//      }
//      ImageLoader il = new ImageLoader();
//      il.load(new java.io.ByteArrayInputStream(baos.toByteArray()));
//      return il.data[0];
//    }
      ...
      case SWT.ICON :
        {
          ...
          if (info.hbmColor == 0) {
            ...
            maskData = new byte[imageSize];
            int lpvMaskBits =
              OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY, imageSize);
            if (OS.IsWinCE)
              SWT.error(SWT.ERROR_NOT_IMPLEMENTED);
            OS.GetDIBits(
              hBitmapDC,
          ...
          ImageData imageData =
            new ImageData(width, height, depth, palette, 4, data);
            ...
            if (isDib) {
              if (OS.IsWinCE) {
                ...
                */
                ...
                for (int i = 0; i < numColors; i++) {
                  pBits[0] =
                    (byte) ((i << (8 - bm.bmBitsPixel)) | (pBits[0] & ~mask));
                  OS.MoveMemory(bm.bmBits, pBits, 1);
                ...
                for (int i = 0; i < rgbs.length; i++) {
                  rgbs[i] =
                    new RGB(
                      colors[colorIndex + 2] & 0xFF,
                      colors[colorIndex + 1] & 0xFF,
  ...
  static int[] init(Device device, Image image, ImageData i) {
    ...
    if ((i.depth == 1 && i.getTransparencyType() != SWT.TRANSPARENCY_MASK)
      || i.depth == 2) {
  ...
  /**
   * Gets the device in use by this image.
   * <p>
   * <b>IMPORTANT:</b> This method is <em>not</em> part of the public
   * API for <code>Image</code>. It is marked public only so that it
   * can be shared within the packages provided by SWT. It is not
  ...
  public Graphics internal_new_GC() {
    return swingHandle.getGraphics();
  }
   ...
   */
  public static Image swing_new(Device device, int type, int handle) {
    if (device == null)
      device = Device.getDevice();
