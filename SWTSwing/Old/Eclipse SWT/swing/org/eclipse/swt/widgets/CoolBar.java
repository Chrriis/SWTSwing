...
import org.eclipse.swt.internal.swing.CCoolBarItem;
...
//  }
  ...
  Container getNewHandle() {
    ...
    // TODO: styles
    coolBar = new CCoolBar();
    if((style & SWT.BORDER) != 0) {
      coolBar.setBorder(javax.swing.BorderFactory.createEtchedBorder());
    }
    return coolBar;
  }
...
//    OS.SendMessage(handle, OS.WM_SETFONT, hFont, 0);
  ...
  void createItem(CoolItem item, int index) {
    ...
    coolBar.add((CCoolBarItem)item.getHandle(), index);
...
//    System.arraycopy(originalItems, index, newOriginals, index + 1,
//                     length - index);
...
//    originalItems = newOriginals;
  ...
  void destroyItem(CoolItem item) {
    ...
    boolean wasVisible = control != null && !control.isDisposed() &&
        control.getVisible();
    ...
    System.arraycopy(originalItems, 0, newOriginals, 0, index);
    System.arraycopy(originalItems, index + 1, newOriginals, index,
                     length - index);
    originalItems = newOriginals;
  }
  ...
  public CoolItem[] getItems() {
    checkWidget();
    int count = coolBar.getItemCount();
    java.util.ArrayList list = new java.util.ArrayList(count);
    for(int i=0; i<count; i++) {
      list.add(items[i]);
    }
    ...
    for(int i=0; i<count; i++) {
      CCoolBarItem item = coolBar.getItem(i);
      for(int j=0; j<list.size(); j++) {
        CoolItem ci = (CoolItem)list.get(j);
        if(ci.swingHandle == item) {
          result[i] = ci;
          list.remove(j);
          break;
        }
...
//    return result;
  ...
  public int indexOf(CoolItem item) {
    ...
    int count = coolBar.getItemCount();
    for(int i=0; i<count; i++) {
      if(coolBar.getItem(i) == item.getHandle()) {
        return i;
      }
    }
    return -1;
//    return OS.SendMessage(handle, OS.RB_IDTOINDEX, item.id, 0);
  ...
  boolean isAcceptingChild(Control control) {
    return false;
  }
