...
public class Label extends Control {
  ...
  Container getNewHandle() {
    if((style & SWT.SEPARATOR) != 0) {
      JSeparator separator = new JSeparator((style & SWT.HORIZONTAL) != 0? JSeparator.HORIZONTAL: JSeparator.VERTICAL);
      // Need to put it in a panel with this layout, otherwise not centred when resizing
...
//  }
  ...
  public void setImage(Image image) {
    ...
    label.setIcon(new ImageIcon(image.swingHandle));
...
//        OS.SWP_NOMOVE;
//    OS.SetWindowPos(handle, 0, 0, 0, rect.right - rect.left,
...
//    OS.InvalidateRect(handle, null, true);
  ...
  public void setText(String string) {
    ...
    if(index != -1) {
      // TODO: find a way to capture mnemonic
      string = string.substring(0, index - 1) + string.substring(index);
    }
    if((style & SWT.WRAP) != 0) {
      string = "<html>" + Utils.escapeHTML(string);
    } else if(string.toLowerCase().startsWith("<html>")) {
      string = " " + string;
    }
