...
public class ScrollBar extends Widget {
  ...
  ScrollBar(Scrollable parent, int style) {
    ...
    if((style & SWT.HORIZONTAL) != 0) {
      swingHandle = parent.getHorizontalScrollBar();
    } else if((style & SWT.VERTICAL) != 0) {
      swingHandle = parent.getVerticalScrollBar();
    }
    swingHandle.addAdjustmentListener(new AdjustmentListener(){
      public void adjustmentValueChanged(AdjustmentEvent e) {
        ...
        switch(e.getAdjustmentType()) {
          case AdjustmentEvent.TRACK:
            ...
            break;
//          case AdjustmentEvent.ADJUSTMENT_FIRST:
...
//    return info.nPos;
  ...
  public Point getSize() {
    checkWidget();
    java.awt.Dimension size = swingHandle.getSize();
...
//		if (OS.IsWinCE) error (SWT.ERROR_NOT_IMPLEMENTED);
  ...
  public void setSelection(int selection) {
    checkWidget();
    swingHandle.setValue(selection);
//    SCROLLINFO info = new SCROLLINFO();
//    info.cbSize = SCROLLINFO.sizeof;
//    int hwnd = hwndScrollBar(), type = scrollBarType();
//    info.fMask = OS.SIF_POS;
...
//    OS.SetScrollInfo(hwnd, type, info, true);
  ...
  public void setThumb(int value) {
    ...
    swingHandle.setVisibleAmount(value);
//    SCROLLINFO info = new SCROLLINFO();
//    info.cbSize = SCROLLINFO.sizeof;
//    int hwnd = hwndScrollBar(), type = scrollBarType();
...
//    }
  ...
  public void setValues(int selection, int minimum, int maximum, int thumb,
    ...
    SCROLLINFO info = new SCROLLINFO();
    info.cbSize = SCROLLINFO.sizeof;
    info.fMask = OS.SIF_POS | OS.SIF_PAGE | OS.SIF_RANGE |
        OS.SIF_DISABLENOSCROLL;
    info.nPos = selection;
    info.nMin = minimum;
    info.nMax = maximum;
    info.nPage = thumb;
...
//		if (OS.IsWinCE) error (SWT.ERROR_NOT_IMPLEMENTED);
  ...
  public void setVisible(boolean visible) {
    checkWidget();
    swingHandle.setVisible(visible);
