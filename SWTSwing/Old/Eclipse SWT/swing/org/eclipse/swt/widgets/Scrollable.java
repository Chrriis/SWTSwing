...
public abstract class Scrollable extends Control {
  ...
  public Rectangle computeTrim(int x, int y, int width, int height) {
    checkWidget();
    java.awt.Rectangle displayRect = getContentPane().getBounds();
    displayRect = SwingUtilities.convertRectangle(getContentPane().getParent(), displayRect, getParent() == null ? null : getParent().getHandle().getParent());
    java.awt.Rectangle handleRect = getHandle().getBounds();
    // This solves the problem of scroll panes. But it assumes the scroll pane
    // is the handle...
    if(getHandle() instanceof javax.swing.JScrollPane) {
      javax.swing.JScrollPane scrollPane = (javax.swing.JScrollPane)getHandle();
      displayRect = scrollPane.getViewport().getParent().getBounds();
    } else if(getHandle() instanceof LayeredContainer && ((LayeredContainer)getHandle()).getComponent() instanceof javax.swing.JScrollPane) {
      javax.swing.JScrollPane scrollPane = (javax.swing.JScrollPane)((LayeredContainer)getHandle()).getComponent();
      displayRect = scrollPane.getViewport().getParent().getBounds();
    }
    int left = displayRect.x - handleRect.x;
    int right = handleRect.width - displayRect.width - left;
    int top = displayRect.y - handleRect.y;
    int bottom = handleRect.height - displayRect.height - top;
...
//    forceResize();
    ...
    if(getHandle() instanceof javax.swing.RootPaneContainer) {
      return new Rectangle(0, 0, bounds.width, bounds.height);
    }
    bounds = SwingUtilities.convertRectangle(contentPane.getParent(), bounds, getHandle());
//    int bLeft = 0;
//    int bTop = 0;
//    int bRight = 0;
//    int bBottom = 0;
//    if(contentPane instanceof JComponent) {
//      Border border = ((JComponent)contentPane).getBorder();
//      if(border != null) {
//        Insets insets = border.getBorderInsets(contentPane);
//        bLeft = insets.left;
//        bTop = insets.top;
//        bRight = insets.right;
//        bBottom = insets.bottom;
//      }
//    }
//    return new Rectangle(bounds.x + bLeft, bounds.y + bTop, bounds.width - bLeft - bRight, bounds.height - bTop - bBottom);
    return new Rectangle(bounds.x, bounds.y, bounds.width, bounds.height);
  }

  // TODO: make those method abstract. Need to implement everywhere first...
