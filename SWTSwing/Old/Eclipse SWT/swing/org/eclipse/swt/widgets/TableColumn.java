...
public class TableColumn extends Item {
  ...
  public int getWidth() {
    ...
    return parent.table.getColumnModel().getColumn(index).getWidth();
...
//    return OS.SendMessage(hwnd, OS.LVM_GETCOLUMNWIDTH, index, 0);
  ...
  public void pack() {
    ...
    int count = parent.table.getRowCount();
    int width = 0;
    // TODO: is there a better way than this hack?
    for(int i=0; i<count; i++) {
      javax.swing.table.TableCellRenderer renderer = parent.table.getCellRenderer(i, index);
      java.awt.Component component = renderer.getTableCellRendererComponent(parent.table, parent.table.getValueAt(i, index), false, false, i, index);
      width = Math.max(width, (int)component.getPreferredSize().getWidth());
    }
    parent.table.getColumnModel().getColumn(index).setPreferredWidth(width + 2);
  }
...
//    OS.SendMessage(hwnd, OS.LVM_SETCOLUMN, index, lvColumn);
  ...
  public void setImage(Image image) {
    ...
    if(image == null) {
      ...
      if((style & SWT.CENTER) == SWT.CENTER) {
        lvColumn.fmt = OS.LVCFMT_CENTER;
      }
      if((style & SWT.RIGHT) == SWT.RIGHT) {
        lvColumn.fmt = OS.LVCFMT_RIGHT;
      }
      OS.SendMessage(hwnd, OS.LVM_SETCOLUMN, index, lvColumn);
    }
  }
...
//    parent.table.getColumnModel().getColumn(index).setResizable(resizable);
    // TODO: find a way to block the resizability of a single column.
    parent.table.getTableHeader().setResizingAllowed(resizable);
  }
  ...
  public void setText(String string) {
    ...
    parent.table.getColumnModel().getColumn(index).setHeaderValue(string);
...
//    }
  ...
  public void setWidth(int width) {
    ...
    parent.table.getColumnModel().getColumn(index).setPreferredWidth(width);
//    int hwnd = parent.handle;
