...
public class Tracker extends Widget {
  ...
  void drawRectangles(Rectangle[] rects) {
    ...
    if(stippled) {
      bandWidth = 3;
      byte[] bits = {
           -86, 0, 85, 0, -86, 0, 85, 0, -86, 0, 85, 0, -86, 0, 85, 0};
    ...
    while(tracking && !cancelled) {
      ...
      switch(message) {
        ...
        case OS.WM_SYSKEYDOWN:
          ...
          break;
        case OS.WM_KEYDOWN:
          int stepSize = OS.GetKeyState(OS.VK_CONTROL) < 0 ? STEPSIZE_SMALL :
              STEPSIZE_LARGE;
  ...
  void resizeRectangles(int xChange, int yChange) {
     ...
     */
    if(xChange < 0 && ((style & SWT.LEFT) != 0) &&
       ((cursorOrientation & SWT.RIGHT) == 0)) {
      cursorOrientation |= SWT.LEFT;
    } else if(xChange > 0 && ((style & SWT.RIGHT) != 0) &&
              ((cursorOrientation & SWT.LEFT) == 0)) {
      cursorOrientation |= SWT.RIGHT;
    } else if(yChange < 0 && ((style & SWT.UP) != 0) &&
              ((cursorOrientation & SWT.DOWN) == 0)) {
      cursorOrientation |= SWT.UP;
    } else if(yChange > 0 && ((style & SWT.DOWN) != 0) &&
              ((cursorOrientation & SWT.UP) == 0)) {
