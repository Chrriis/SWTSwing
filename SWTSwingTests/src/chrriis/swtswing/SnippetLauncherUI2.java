...
public class SnippetLauncherUI2 extends JFrame {

  protected static final ImageIcon APPLICATION_ICON = new ImageIcon(SnippetLauncherUI2.class.getResource("resources/SWTSwingIcon32x32.gif"));
  protected static final ImageIcon SOURCE_ICON = new ImageIcon(SnippetLauncherUI2.class.getResource("resources/source.gif"));

  protected static final Icon SWT_ICON = new ImageIcon(SnippetLauncherUI2.class.getResource("resources/SWT100x55.png"));
  protected static final Icon SWTSWING_ICON = new ImageIcon(SnippetLauncherUI2.class.getResource("resources/SWTSwing100x55.png"));

  protected static final Icon ROOT_ICON = new ImageIcon(SnippetLauncherUI2.class.getResource("resources/snippets.gif"));
  protected static final Icon UNSUPPORTED_ICON = new ImageIcon(SnippetLauncherUI2.class.getResource("resources/Unsupported.gif"));
  protected static final Icon WORKING_ICON = new ImageIcon(SnippetLauncherUI2.class.getResource("resources/Working.gif"));
  protected static final Icon PARTIALLY_WORKING_ICON = new ImageIcon(SnippetLauncherUI2.class.getResource("resources/PartiallyWorking.gif"));
  protected static final Icon NOT_WORKING_ICON = new ImageIcon(SnippetLauncherUI2.class.getResource("resources/NotWorking.gif"));
  ...
  protected JToggleButton swtToogleButton;
  protected JToggleButton swtswingToogleButton;
  
  protected String filterText;

  public SnippetLauncherUI2() {
    ...
    addWindowListener(new WindowAdapter() {
      public void windowClosing(WindowEvent e) {
        if(processSWT != null || processSWTSwing != null) {
          try {
            processSWT.destroy();
            processSWTSwing.destroy();
          } catch(Exception ex) {
          }
        }
      }
    ...
    swtToogleButton 		= new JToggleButton(SWT_ICON);
    swtswingToogleButton 	= new JToggleButton(SWTSWING_ICON);
    swtswingToogleButton.setSelected(true);
    ActionListener exclusiveSelection = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (swtToogleButton.isSelected() == false && swtswingToogleButton.isSelected() == false) {
					if (e.getSource().equals(swtToogleButton)) swtswingToogleButton.setSelected(true);
					else swtToogleButton.setSelected(true);
				}
			}
	    };
	swtToogleButton.addActionListener(exclusiveSelection);
	swtswingToogleButton.addActionListener(exclusiveSelection);
...
//    swtChoicePane.add(swtComboBox);
    swtChoicePane.add(swtswingToogleButton);
    Dimension d = new Dimension(10,10);
	swtChoicePane.add(new Box.Filler(d ,d,d));
    swtChoicePane.add(swtToogleButton);
  ...
  protected Process processSWT;
  protected Process processSWTSwing;
  ...
  protected void launchSnippet() {
    ...
    if(processSWT != null || processSWTSwing != null) {
      if (processSWT!=null) 		processSWT.destroy();
	  if (processSWTSwing!=null) 	processSWTSwing.destroy();
      return;
    }
    ...
    try {
			if (swtToogleButton.isSelected()) {
				startProcess(parameterSetup_SWT(), processSWT);
			}
			if (swtswingToogleButton.isSelected()) {
				startProcess(parameterSetup_SWTSwing(), processSWTSwing);
			}
	} catch (Exception e) {
    ...
    new Thread() {
			public void run() {
				while (true) {
					...
					try {
						if (processSWT!=null) {
							processSWT.exitValue();
							processSWT = null;
						}
						if (processSWTSwing!=null) {
							processSWTSwing.exitValue();
							processSWTSwing = null;
						}
						SwingUtilities.invokeLater(new Runnable() {
							public void run() {
								launchButton.setText(LAUNCH_TEXT);
								System.out.println(LAUNCH_TEXT);
							}
						});
						break;
					} catch (Exception e) {
	...
	private void startProcess(ArrayList parameterList, Process p) throws IOException {
...
private ArrayList parameterSetup_SWTSwing() {
...
private ArrayList parameterSetup_SWT() {
	ArrayList parameterList = new ArrayList();
	String pathSeparator = System.getProperty("path.separator");
  ...
  public static void main(String[] args) {
    ...
    new SnippetLauncherUI2().setVisible(true);
  }
